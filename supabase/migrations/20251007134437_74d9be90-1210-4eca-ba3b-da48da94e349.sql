...
CREATE TABLE IF NOT EXISTS public.challenge_instances (
  ...
  UNIQUE (challenge_code, user_id, window_start)
);
...
BEGIN
  FOR v_instance IN
    SELECT ci.id, ci.challenge_code, c.conditions, c.reward, c.unlock, COALESCE(ucp.progress, '{}'::jsonb) as progress
  ...
  LOOP
    ...
    IF v_instance.conditions->>'type' = 'days_in_period' THEN
      v_days_marked := COALESCE(v_progress->'days_marked', '[]'::jsonb);
      v_is_new_day := NOT (v_days_marked ? p_local_date::text);
        ...
        IF v_days_done >= (v_instance.conditions->>'min_days')::int THEN
          ...
          IF NOT COALESCE((v_instance.reward->>'claimable')::boolean, false) THEN
            ...
            IF v_instance.reward->>'badge_code' IS NOT NULL THEN
              INSERT INTO public.user_medals (user_id, medal_id, earned_at) SELECT p_user_id, m.id, now() FROM public.medals m WHERE m.code = v_instance.reward->>'badge_code' ON CONFLICT (user_id, medal_id) DO NOTHING;
            END IF;
          ...
          IF v_instance.unlock IS NOT NULL AND v_instance.unlock->'on_complete_unlock_codes' IS NOT NULL THEN
            ...
            FROM jsonb_array_elements_text(v_instance.unlock->'on_complete_unlock_codes') AS unlock_code JOIN public.challenges c2 ON c2.code = unlock_code::text
            ON CONFLICT (challenge_code, user_id, window_start) DO NOTHING;
          END IF;
        ELSE
...
DECLARE v_user_tz TEXT; v_local_date DATE; v_today_logs INTEGER; v_today_minutes INTEGER; v_existing_log RECORD; v_new_log_id UUID;
  v_xp_today_before INTEGER; v_xp_gained INTEGER; v_xp_today_after INTEGER; v_xp_total INTEGER;
  v_current_streak INTEGER; v_best_streak INTEGER; v_yesterday_str DATE; v_last_active_date DATE;
  v_league_id UUID; v_league_status league_status; v_league_xp_week INTEGER;
  v_first_practice_medal_id UUID; v_daily_cap_medal_id UUID; v_practice_count INTEGER; v_result JSONB;
BEGIN
  SELECT tz INTO v_user_tz FROM public.profiles WHERE id = p_user_id; IF v_user_tz IS NULL THEN v_user_tz := 'Asia/Tehran'; END IF;
  ...
  VALUES (p_user_id, p_minutes, p_note, v_local_date, v_local_date, p_idempotency_key) RETURNING id INTO v_new_log_id;
  ...
  INSERT INTO public.xp_events (user_id, source, delta, practice_log_id, local_date) VALUES (p_user_id, 'practice', v_xp_gained, v_new_log_id, v_local_date);
  ...
  SELECT current_streak, best_streak, last_active_local_date INTO v_current_streak, v_best_streak, v_last_active_date FROM public.streaks WHERE user_id = p_user_id;
  v_yesterday_str := v_local_date - INTERVAL '1 day';
  IF v_last_active_date IS NULL THEN v_current_streak := 1; v_best_streak := 1;
  ELSIF v_last_active_date = v_local_date THEN NULL;
  ELSIF v_last_active_date = v_yesterday_str THEN v_current_streak := v_current_streak + 1; v_best_streak := GREATEST(v_best_streak, v_current_streak);
...
BEGIN
  ...
  FROM public.challenge_instances ci JOIN public.challenges c ON c.code = ci.challenge_code LEFT JOIN public.user_challenge_progress ucp ON ucp.instance_id = ci.id AND ucp.user_id = p_user_id
  WHERE ci.user_id = p_user_id AND ci.status = 'open' AND c.status = 'active' AND (ci.window_start IS NULL OR v_today >= ci.window_start)
    AND (ci.window_end IS NULL OR v_today <= ci.window_end) AND (ucp.is_completed IS NULL OR ucp.is_completed = false);
...
CREATE OR REPLACE FUNCTION public.rpc_claim_challenge_reward(p_user_id uuid, p_instance_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
...
BEGIN
  ...
  IF v_progress.reward->>'badge_code' IS NOT NULL THEN
    INSERT INTO public.user_medals (user_id, medal_id, earned_at) SELECT p_user_id, m.id, now() FROM public.medals m WHERE m.code = v_progress.reward->>'badge_code' ON CONFLICT (user_id, medal_id) DO NOTHING; v_badge_granted := true;
  END IF;
...
CREATE OR REPLACE FUNCTION public.rpc_challenge_rollover_periodic()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public' AS $$
...
BEGIN
  ...
  INSERT INTO public.challenge_instances (challenge_code, user_id, window_start, window_end, status) SELECT c.code, p.id, v_week_start, v_week_end, 'open'
  FROM public.challenges c CROSS JOIN public.profiles p WHERE c.kind = 'periodic' AND c.period = 'week' AND c.status = 'active' AND c.auto_enroll = true
  ON CONFLICT (challenge_code, user_id, window_start) DO NOTHING;
  INSERT INTO public.challenge_instances (challenge_code, user_id, window_start, window_end, status) SELECT c.code, p.id, v_month_start, v_month_end, 'open'
  FROM public.challenges c CROSS JOIN public.profiles p WHERE c.kind = 'periodic' AND c.period = 'month' AND c.status = 'active' AND c.auto_enroll = true
  ON CONFLICT (challenge_code, user_id, window_start) DO NOTHING;
...
VALUES
  ('weekly_5of7', 'چالش هفتگی ۵ از ۷', 'periodic', 'week', true, '{"type":"days_in_period","min_days":5,"period":"week"}'::jsonb, '{"xp":100,"badge_code":"weekly_challenge","claimable":false}'::jsonb, NULL, 'active'),
  ('streak_10_days', 'استمرار ۱۰ روزه', 'rolling', 'none', true, '{"type":"streak","min_days":10,"source":"practice"}'::jsonb, '{"xp":150,"badge_code":"streak_10","claimable":false}'::jsonb, '{"on_complete_unlock_codes":["monthly_champion"]}'::jsonb, 'active'),
  ('monthly_champion', 'قهرمان ماه', 'periodic', 'month', false, '{"type":"days_in_period","min_days":20,"period":"month"}'::jsonb, '{"xp":300,"badge_code":"monthly_champion","claimable":true}'::jsonb, NULL, 'active')
ON CONFLICT (code) DO UPDATE SET title = EXCLUDED.title, kind = EXCLUDED.kind, period = EXCLUDED.period, auto_enroll = EXCLUDED.auto_enroll, conditions = EXCLUDED.conditions, reward = EXCLUDED.reward, unlock = EXCLUDED.unlock, status = EXCLUDED.status;

INSERT INTO public.challenge_instances (challenge_code, user_id, window_start, window_end, status) SELECT 'weekly_5of7', p.id,
  (now() AT TIME ZONE 'Asia/Tehran')::date - ((EXTRACT(DOW FROM (now() AT TIME ZONE 'Asia/Tehran')::date)::int + 1) % 7),
...
INSERT INTO public.challenge_instances (challenge_code, user_id, window_start, window_end, status) SELECT 'streak_10_days', p.id, NULL, NULL, 'open'
FROM public.profiles p ON CONFLICT (challenge_code, user_id, window_start) DO NOTHING;
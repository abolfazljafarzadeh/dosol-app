...
BEGIN
  ...
  SELECT 
    total_xp,
    (SELECT COUNT(*) + 1 FROM public.xp_counters xc2 WHERE xc2.total_xp > xc.total_xp) as rank
  INTO v_user_xp, v_user_rank
  FROM public.xp_counters xc
  WHERE xc.user_id = p_user_id;

  -- Get top 3 users
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', p.id,
      'name', COALESCE(p.first_name || ' ' || COALESCE(p.last_name, ''), 'کاربر'),
      'instrument', COALESCE(p.instrument, 'پیانو'),
      'totalPoints', xc.total_xp,
      'rank', row_number
    )
  )
  INTO v_top_three
  FROM (
    SELECT p.*, xc.total_xp, ROW_NUMBER() OVER (ORDER BY xc.total_xp DESC) as row_number

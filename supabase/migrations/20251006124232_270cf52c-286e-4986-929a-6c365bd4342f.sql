-- ✅ Create atomic RPC function for practice logging
...
BEGIN
  ...
  -- 2. Calculate local_date
  v_local_date := (p_now_utc AT TIME ZONE v_user_tz)::DATE;

  -- 3. Check for idempotent request
  SELECT pl.id, pl.local_date
  INTO v_existing_log
  ...
  IF FOUND THEN
    ...
    SELECT total_xp INTO v_xp_total
    ...
    SELECT delta INTO v_xp_gained
    FROM public.xp_events
    WHERE user_id = p_user_id AND practice_log_id = v_existing_log.id
    LIMIT 1;
  ...
  INTO v_today_logs, v_today_minutes
  ...
  -- 7. Calculate XP (with daily cap of 160)
  ...
  v_xp_gained := LEAST(
    (p_minutes / 15) * 10,
    GREATEST(0, 160 - v_xp_today_before)
  );
  ...
  SET total_xp = xp_counters.total_xp + v_xp_gained,
  ...
  -- 11. Check and award medals
  -- First practice medal
  SELECT COUNT(*) INTO v_practice_count
  FROM public.practice_logs
  WHERE user_id = p_user_id;

  IF v_practice_count = 1 THEN
    SELECT id INTO v_first_practice_medal_id
    FROM public.medals
    WHERE code = 'first_practice'
    LIMIT 1;
  ...
  IF v_xp_today_after >= 160 THEN
    SELECT id INTO v_daily_cap_medal_id
    FROM public.medals
    WHERE code = 'daily_cap_reached'
    LIMIT 1;
  ...
  -- 12. Update league scores if member of active league
  SELECT lm.league_id, wl.status
  INTO v_league_id, v_league_status
  ...
  IF v_league_id IS NOT NULL THEN
    IF v_league_status = 'locked' OR v_league_status = 'finalized' THEN
      RETURN jsonb_build_object(
        'ok', false,
        'code', 'LEAGUE_LOCKED',
        'message', 'این لیگ بسته شده و نمی‌توانید امتیاز اضافه کنید.'
      );
    END IF;

    -- Update league_scores only (single source of truth)
  ...
  v_result := jsonb_build_object(
    ...
    'challenge', NULL,
    'league', CASE
      WHEN v_league_id IS NOT NULL THEN
        jsonb_build_object('id', v_league_id, 'xpWeek', v_league_xp_week, 'rank', NULL)
      ELSE NULL
    END
  );

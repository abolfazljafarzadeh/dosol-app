-- Update rpc_evaluate_challenges to create notifications when challenge is completed
...
BEGIN
  FOR v_instance IN SELECT ci.id, ci.challenge_code, c.conditions, c.reward, c.unlock, c.title, COALESCE(ucp.progress, '{}'::jsonb) as progress
    ...
    IF v_instance.conditions->>'type' = 'days_in_period' THEN
      ...
      IF v_is_new_day THEN
        ...
        IF v_days_done >= (v_instance.conditions->>'min_days')::int THEN
          ...
          INSERT INTO public.notifications (user_id, type, status, payload)
          VALUES (p_user_id, 'challenge_completed', 'queued', jsonb_build_object(
            'challenge_code', v_instance.challenge_code,
            'challenge_title', v_instance.title,
            'instance_id', v_instance.id,
            'is_claimable', COALESCE((v_instance.reward->>'claimable')::boolean, false),
            'message', 'تبریک! چالش "' || v_instance.title || '" رو کامل کردی 🏅'
          ));
